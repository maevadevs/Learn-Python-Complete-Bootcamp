...
#   3. Keep or remove the jupyter notebook markdown (comments) in the .py file (Optional)
# PARAMETERS:
...
#       > .\nb2py.ps1 notes notebook *> log.txt
#       > .\nb2py.ps1 not* not* -nomypy
#       > .\nb2py.ps1 not* not* -nomypy -keepcells -keepmd
################################################################################################################
# 1.0.0     2023-03-15      Maeva Ralafiarindaza    :   INITIAL RELEASE
# 1.1.0     2023-03-22      Maeva Ralafiarindaza    :   Allow wildcard * in args and resolve
#                                                       Add -nomypy, -keepcells, and -keepmd optional parameters
################################################################################################################
# File Parameters
param (
...
$PROJ_DIR = "00.Complete-Python-Fundamentals"
################################################################################################################
...
# Early exit if $IPYNB_FILE_PATH does not exit
...
# If the IPYNB_FILE_PATH is valid, run build command
...
# Convert .ipynb to .py with jupyter nbconvert
Start-Process -NoNewWindow -Wait -FilePath ".\venv\Scripts\python.exe" -ArgumentList "-m jupyter nbconvert --to python $($IPYNB_FILE_PATH) --PythonExporter.exclude_markdown=$(-not $keepmd)"
...
if (-not $keepcells)
{
    ...
    $py_contents = $py_contents -Replace "#\sIn\[\d+\]:\s*\n*", ""
...
if (-not $nomypy)
{
    ...
    Start-Process -NoNewWindow -Wait -FilePath ".\venv\Scripts\python.exe" -ArgumentList "-m mypy $($PY_FILE_PATH)"
}
